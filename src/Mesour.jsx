...
export default class Mesour
{
	...
	constructor(oldObject)
	{
		...
		jQuery(document).on('click.mesour-ajax', '[data-mesour=ajax]:not(form)', function (e) {
			e.preventDefault();
			jQuery.get(jQuery(this).attr('href')).complete(Mesour.redrawCallback);
		});
	}
	 ...
	 */
	createWidget(name, instance)
	{
		if (this.widgets[name]) {
			throw new Error('Widget with name ' + name + ' is already loaded.');
		}
		if (instance.push) {
		...
		if (typeof instance.ready === 'function') {
			jQuery(document).on('ready.' + name, (event) => {
				instance.ready(event);
			});
		}
		if (typeof instance.live === 'function') {
			jQuery(document).on('ready.' + name + '-ajax', (event) => {
				instance.live(event);
			});
			jQuery(window).on('ajaxComplete.' + name, (event, request, options) => {
				instance.live(event, request, options);
			});
		}
		...
		if (this.oldObject[name]) {
			...
			for (var i = 0; i < arr.length; i++) {
				...
				instance[method].apply(instance, Helper.cleanArray(arr[i]));
			}
		}
		this[name] = this.widgets[name] = instance;
		return instance;
	}
	 ...
	 */
	static redrawCallback(response) {
